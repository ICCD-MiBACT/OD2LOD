...
public class ServiceReader implements DocumentReader {
 ...
 private Document readDocument(String request) throws Exception { int tryCount = maxTry;
  for (;;) { //System.out.println("GET " + request);
   try { URL url = new URL(request);
    ...
    if (--tryCount==0) { throw e; }  Thread.sleep(tryAfter * 1000);    
   }
  }
 }
 public ServiceReader(String queryUri, String dataUri, String countPath) throws Exception {
  ...
  count = Integer.parseInt((String)xPath.evaluate(countPath, doc, XPathConstants.STRING)); System.out.println("STATUS - got " + count + " cards reading @" + queryUri);
  for (int lastPos=0;;) { int pos = dataUri.indexOf("$(/", lastPos); if (pos<0) break;
