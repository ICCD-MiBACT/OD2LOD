...
<!--
	    PVCN, PVCJ, LDCZ, CMPE campi esclusi perché non presenti in scheda OA.
	   
	   Inserita gestione dampi di gruppo (AUT, ATB): si ripetono AUT e ATB se si trovano valori ripetuti.
       -->
  ...
  <record>
    ...
    <schede>
     ...
     <xsl:element name="PST">
      ...
      <xsl:element name="OG">
       ...
       <xsl:for-each select="cell[@name='SGTI']">
	  ...
	  <xsl:for-each select="cell[@name='COLD']">
	   <xsl:element name="UB">
       <xsl:attribute name="hint">UBICAZIONE E DATI PATRIMONIALI</xsl:attribute>
        ...
        <xsl:attribute name="hint">COLLEZIONI</xsl:attribute>
			<xsl:element name="COLD">
			 <xsl:attribute name="hint">Denominazione</xsl:attribute><xsl:value-of select="."/></xsl:element>
	   </xsl:element>
      </xsl:element>
	  </xsl:for-each>
   ...
   <xsl:variable name="max" select="max((count(cell[@name='DTZG']), count(cell[@name='DTZS']), count(cell[@name='DTSI']), count(cell[@name='DTSV']), count(cell[@name='DTSF']), count(cell[@name='DTSL'])))"/>
    ...
    <xsl:variable name="pos" select="."/>
	 ...
	 <xsl:for-each select="1 to $max">
      ...
      <xsl:if test="cell[@name='DESO']|cell[@name='UTF']|cell[@name='UTM']">
       ...
       <xsl:element name="CO">
		...
		<xsl:if test="cell[@name='STCC']|cell[@name='STCD']">
			<xsl:variable name="max" select="max((count(cell[@name='STCC']), count(cell[@name='STCD'])))"/>
			<xsl:variable name="stccCells" select="cell[@name='STCC']"/>
			<xsl:variable name="stcdCells" select="cell[@name='STCD']"/>
        ...
        <xsl:element name="STC">
         <xsl:attribute name="hint">STATO DI CONSERVAZIONE</xsl:attribute>
			...
			<xsl:if test="$stccCells[$pos]">
				<xsl:element name="STCC">
				<xsl:attribute name="hint">Stato di conservazione</xsl:attribute>
					<xsl:value-of select="$stccCells[$pos]"/>
				</xsl:element>
			</xsl:if>
			<xsl:if test="$stcdCells[$pos]">
				<xsl:element name="STCD">
				<xsl:attribute name="hint">Data</xsl:attribute>
					<xsl:value-of select="$stcdCells[$pos]"/>
				</xsl:element>
			</xsl:if>
     ...
     <xsl:when test="cell[@name='location'][starts-with(.,'POINT')]"><!-- After --><!-- eg: "POINT (9.0294865666 45.628287118)" -->
     <xsl:element name="harvesting">
      <xsl:element name="geocoding">
       <xsl:element name="x"><xsl:value-of select="substring-before(substring-after(cell[@name='location'],'('),' ')"/></xsl:element>		
       <xsl:element name="y"><xsl:value-of select="substring-before(substring-after(substring-after(cell[@name='location'],'('),' '),')')"/></xsl:element>
      </xsl:element>
     </xsl:element>
     </xsl:when>
