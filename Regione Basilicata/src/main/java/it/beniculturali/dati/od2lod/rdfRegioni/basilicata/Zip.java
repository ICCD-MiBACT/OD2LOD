...
public class Zip { // flat zip filesystem
  static List<Path> list(FileSystem fs) throws IOException {
  ...
  static void close(FileSystem fs) throws IOException {
    if (fs != null) fs.close();
  }

  @SuppressWarnings("serial")
  static FileSystem create(String outFolder, String name) throws IOException {
    return FileSystems.newFileSystem(URI.create("jar:" + new File(outFolder, name + ".zip").toURI()), new HashMap<String, String>() {
      {
        put("create", "true");
      }
    });
  ...
  static FileSystem open(File z) throws IOException {
  ...
  static byte[] bytes(FileSystem fs, String name) throws IOException {
    Path f = fs.getPath("/", name);
